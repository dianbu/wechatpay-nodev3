...
import { ValidationException, CipherException, CertificateException } from './exceptions';
import { AutoCertificateManager } from './certificate-manager';
...
export class NotificationParser {
  ...
  private decryptResource<T>(resource: NotificationResource): T {
    ...
    try {
      ...
      const decryptedStr = decoded + final;
 ...
 * 转账批次通知数据
 */
export interface TransferBatchNotification {
  /** 商户号 */
  mchid: string;
  /** 商家批次单号 */
  out_batch_no: string;
 ...
 * 分账结果通知数据
 */
export interface ProfitSharingNotification {
  /** 商户号 */
  mchid: string;
  /** 微信订单号 */
  transaction_id: string;
