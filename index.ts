...
class Pay extends Base {
   ...
   * 获取 HTTP 服务（用于创建业务服务）
   */
  public getHttpService(): IPayRequest {
   ...
   * 获取 APIv3 密钥
   */
  public getApiV3Key(): string | undefined {
    return this.key;
  }
  /**
   * 获取微信平台key
  ...
  /**
   * 敏感信息加密（使用 RSA-OAEP with SHA-256，符合微信支付官方标准）
   * @param str 敏感信息字段（如用户的住址、银行卡号、手机号码等）
   * @param wxPublicKey 微信支付公钥
   * @param padding 填充模式，默认为 RSA_PKCS1_OAEP_PADDING
  ...
  public publicEncrypt(str: string, wxPublicKey: Buffer, padding = crypto.constants.RSA_PKCS1_OAEP_PADDING) {
    if (![crypto.constants.RSA_PKCS1_PADDING, crypto.constants.RSA_PKCS1_OAEP_PADDING].includes(padding)) {
      throw new Error(`不支持的填充模式 (${padding})，仅支持 RSA_PKCS1_OAEP_PADDING 或 RSA_PKCS1_PADDING`);
    }
    // 使用 SHA-256 符合微信支付官方要求
    const encrypted = crypto.publicEncrypt(
      { 
        key: wxPublicKey, 
        padding, 
        oaepHash: 'sha256'  // 修复：使用 SHA-256 替代 SHA-1
      }, 
